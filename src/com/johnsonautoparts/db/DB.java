 ...
 *
 * <p>Class for handling database connections and calls when needed
 */
...
public class DB {
  /**
   * Method to instantiate a connection to the database and store the connection object with the
   * session.
   *
   * <p>This class is not part of the course that needs to be reviewed or updated. It is only here
  ...
  public static Connection getDbConnection(HttpSession session) throws DBException {
    try {
      ...
      AppLogger.log(
          "DB exception because class not found. ID: "
              + session.getId()
              + " Error: "
              + cnfe.getMessage());
      ...
      AppLogger.log(
          "DB exception accessing class. ID: " + session.getId() + " Error: " + ce.getMessage());

      throw new DBException(ce);
    } catch (SQLException se) {
      AppLogger.log(
          "DB exception getting connection. ID: " + session.getId() + " Error: " + se.getMessage());
  ...
  public static void closeDbConnection(HttpSession session) throws DBException {
    try {
      ...
      AppLogger.log(
          "DB not destroyed with error. ID: " + session.getId() + "Error: " + se.getMessage());

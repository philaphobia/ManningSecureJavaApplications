...
public class CSRFValidationFilter implements Filter {
	public static final String TARGET_ORIGIN="http://localhost:8080";
    /**
     ...
     * Logger
     */
    //private static final Logger LOG = AppLogger.class;
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        if (this.isBlank(source)) {
            ...
            if (this.isBlank(source)) {
                ...
                AppLogger.log(accessDeniedReason);
        ...
        if (tokenCookie == null || this.isBlank(tokenCookie.getValue())) {
        	AppLogger.log("CSRFValidationFilter: CSRF cookie absent or value is null/empty so we provide one and return an HTTP NO_CONTENT response !");
        	...
        	String csrfCookieExpectedName = this.determineCookieName(httpReq);
        	AppLogger.log("Expected Cookine name: " + csrfCookieExpectedName);
    ...
    public void init(FilterConfig filterConfig) throws ServletException {
        ...
        try {
            this.targetOrigin = new URL(TARGET_ORIGIN);
        } catch (MalformedURLException e) {
            AppLogger.log("Cannot init the filter: " + e.getMessage());
            throw new ServletException(e);
        }
        AppLogger.log("CSRFValidationFilter: Filter init, set expected target origin to " + this.targetOrigin);
    }
    ...
    @Override
    public void destroy() {
        AppLogger.log("CSRFValidationFilter: Filter shutdown");
    }
